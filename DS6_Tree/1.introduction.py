'''
    ...
    一、树相关的概念：
        ...
        边：边是树的另一个基本部分。边连接两个节点以显示他们之间的存在关系。每个节点（除了跟之外）都恰好从另一个节点传入连接。每个节点都可以具有多个输出边
        跟：树的跟是树中唯一没有传入边的节点
        路径：路径是由边连接节点的有序列表
        子节点：具有来自相同传入边的节点C的集合成为该节点的子节点
        父节点：具有和他相同传入边的所有连接的节点为父节点
    ...
    二、树的定义：
        定义一：树由一组节点和一组连接节点的边组成
            树的一个节点被指定为跟节点
            除了根节点之外，每个节点N通过一个其他节点P的边连接，其中，P是n的父节点
            从根路径遍历到每个节点路径是唯一
            如果数中的每个节点最多有两个子节点，我们说概述是一个‘二叉树’
        定义二（递归定义）：
            树是空的，或者由一个根节点和0个或者多个子树组成，每个子树也是一棵树。每个子树的节点通过边连接到父树的跟节点
    三、树的表示：
        1.列表['html',
               ['head'  ['title',[],[],['meta'],[]]]
               ['body'] [''],[]]
...
r.insertRight('c')
print(r.getRightChld())
print(r.getRightChld().getRootVal())
r.getRightChld().setRootVal('hello')
print(r.getRightChld().getRootVal())
