# 注册牛客网， LeetCode(必刷)
# 如果a+b+c = 1000 ，  并且a^2 + b^2 = c^2 ,求出所有的a ,b ,c 可能的组合
# 解决同一个问题，时间段的算法更好
...
# print("运行时间差：%f"%(end_time-start_time))
...
'''
    ...
    2.算法的五大特性：
        输入：算法有0个或者多个输入
        输出：算法至少有1个或者多个输出
        有穷性：算法要在有限的步骤之后会自动结束而不会无限循环，并且每一个步骤可以在可接受的时间内完成
        确定性：算法中的每一个步骤都要有确定的含义，不会出现二义性
        可行性：算法的每一步都是可行的（每一步都能够执行有限的次数完成）
    3.算法的效率的衡量
        实现算法程序的执行时间可以反映出来算法的效率
        单纯依靠运行时间来比较算法的优劣不一定是客观准确的（程序的运行离不开计算机环境，所以和硬件，操作系统都有关系）
    4.最终算法用什么去衡量？
        时间复杂度
    5.表示法：大O记法
        假设计算机执行算法每个基本操作的时间是固定的一个时间单位，那么有多少个基本操作就代表会花费多少的时间
        虽然对于不同的机器环境而言，确切的时间单位是不同的，但是对于算法进行多少个基本操作规模数量级上市相同的，因此，可以忽略机器环境的影响而客观的反应算法的时间效率
        对于算法的时间效率，用‘大O记法’
